...
// MARK: - Characters HTTP request endpoint

/// Перечень `endpoint'ов` для `HTTP`-запросов в
/// `API` персонажей из вселенной `"Rick and Morty"`.
enum CharactersHTTPRequestEndpoint: HTTPRequestEndpoint {
    
    // MARK: Cases
    
    /// `Endpoint` для получения полной информации о персонаже по переданному идентификатору.
    /// - Parameter id: уникальный идентификатор персонажа.
    /// - Returns: `endpoint` для запроса формата `/api/character/{id}`.
    case getCharacterWitId(_ id: Int)
    /// `Endpoint` для получения полной информации о персонажах по переданному списку идентификаторов.
    /// - Parameter ids: список идентификаторов персонажей.
    /// - Returns: `endpoint` для запроса формата `/api/character/[id1,id2,...]`.
    case getCharactersWithIds(_ ids: [Int])
    /// `Endpoint` для получения полной информации о персонажах с учетом фильтрации.
    /// - Parameter filter: параметры фильтрации.
    /// - Returns: `endpoint` для запроса формата `/api/character` с `query`-параметрами.
    case getCharactersWithFilter(_ filter: CharactersHTTPRequestFilter?)
